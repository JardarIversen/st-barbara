...
- `/nb/…` er norsk original, `/en/…` er kontrollert engelsk. Øvrige støttede språk har egne adresser, f.eks. `/pl/…` og `/es/…`, med automatisk oversettelse fra engelsk. Sidestier og hendelses-ID-er er felles.
- Språkvalget lagres i `site_language` i 365 dager (`Secure`, `SameSite=Lax`, `Path=/`). Bare et eksplisitt valg i språkvelgeren eller oversettelsesvarselet endrer preferansen. Adresser uten språk bruker den lagrede preferansen, ellers norsk. Slike videresendinger skal ikke mellomlagres i delte cacher.
- Språket i en uttrykkelig URL vinner alltid over informasjonskapsler. En direkte lenke til `/en/…` viser derfor engelsk selv med lagret polsk preferanse. Eldre `/en/…?translate=pl`-lenker videresendes til `/pl/…`. Google-kapselen `googtrans` er bare et teknisk hjelpemiddel, aldri kilden til nettstedets språkvalg.
- Automatiske språkversjoner har `noindex, follow` i både HTTP-header og metadata, og engelsk kilde som canonical. Bare norsk og engelsk oppgis som `hreflang`-alternativer. Dokumentets språk er engelsk inntil automatisk oversettelse er fullført; varselet og språkvelgeren beholder sitt faktiske språk.
- Språkvelgeren beholder sidesti, søkeparametere og anker. Norsk og engelsk bruker aldri Google Translate. Andre språk viser engelsk på sin egen språkadresse mens oversettelsen lastes, med et synlig varsel. Navigasjon på disse sidene laster hele dokumentet for å unngå konflikt mellom Google Translate og React. Hvis Google ikke laster, beholdes engelsk med en feilmelding.
...
- Søk matcher språkets eget navn, norsk navn, engelsk navn og kode. Støttede koder som `zh-CN` bevares ved navigasjon. Menyen selv skal ikke maskinoversettes.
