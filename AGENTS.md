...
2. Kjør `npm run bulletin:inspect -- --json` i `studio-st.-barbara-church/`. Gjenbruk eksisterende `sourceKey` for samme sted, messeplan, hendelse eller kunngjøring. Gjennomgå også aktive kunngjøringer: vurder sluttdato for hver ny/oppdatert sak og avslutt eldre saker som er gjennomført, erstattet eller ikke lenger aktuelle. Ta disse endringene med i manifestet.
...
- `events`: tidsbestemt innhold som messe, menighetsråd, pilegrimstur, aktivitet, sosialt arrangement eller annet. En særmesse kan både være en `event` og være koblet fra et `massException`. Ved pilegrimstur med messe: lag turen som forelder og messen som underhendelse. Hendelser med `registrationDeadline` vises også under Kunngjøringer frem til fristen. Agenten kan sette både `promotionFrom` og `promotionUntil` (ISO 8601) for en begrunnet fremhevingsperiode, også uten påmelding. Denne perioden erstatter automatikk basert på påmeldingsfrist. Avlyste, utsatte og avsluttede hendelser fremheves ikke. Vanlige messer fremheves bare eksplisitt. Gjenbruk hendelsens tittel, tekst og oversettelser; ikke opprett en duplikat-kunngjøring.
...
- `massTexts`: lesningene gjelder bare den navngitte søndagen eller høytiden på bladets forside, normalt ved norsk søndagsmesse kl. 11 i Kongsberg. Følg kontrollen av lesningsdato nedenfor. Ikke koble norsk tekst til engelsk messe kl. 13.
...
- Katekese-informasjonen ligger på artikkelen med `pagePath: /katekese`. Lenker til denne oppføringen skal gå til informasjonssiden, også fra forsiden og innleggsarkivet. `body` er innledningen; `catechesis` inneholder påmeldingsbeskjed (`summary`), aldersgrupper (`groups`), undervisningstider (`sessions`), bekreftede datoer (`sessionDates`), kontakter og øvrige tilbud (`otherOfferings`). Ikke kopier tider og datoer tilbake til brødteksten. Datoene for 7–12-åringene er felles for begge undervisningsgruppene. Ikke beregn oppmøtedatoer for 3–7-åringene ut fra den fleksible ukeplanen. Hvert objekt kan ha `translations.en` for sine tekstfelt; datoer, tider og kontaktadresser er felles. Behold `_key` ved oppdatering av samme gruppe eller tilbud.
- `places`: gjenbrukbare kirker og arrangementssteder med adresse/kartlenke. Opprett bare når stedet ikke finnes fra før.

## Lesningsdato og gjenbruk

- Finn datoen fra søndagsnummeret/høytiden på forsiden og det aktuelle året. Bekreft med en katolsk liturgisk kalender og kontroller mot eventuell datofesting i e-posten. «Neste søndag» er relativt til utsendelsen, ikke dagen agenten behandler bladet. Ikke bruk mottaksdato eller siste søndag i tabellen som lesningsdato.
- Skill `massDate` fra bladets `coversFrom`/`coversUntil`: «Messetider kommende uke» kan dekke neste uke og flere søndager, mens lesningene bare gjelder forsidesøndagen. En PDF kan derfor være kilde til en senere messetid uten at lesningene gjelder den messen. Ikke fall tilbake til siste tilgjengelige lesninger på datoer uten egen messetekst.
- Lagre teksten én gang i `massText`, koblet med `massDate`, `massSchedule` og `sourceBulletin`. Ved feil dato eller referanse: rett bare de aktuelle feltene på eksisterende dokument, behold `_id` og stabil `sourceKey`, og oppdater den eksisterende manifestoppføringen tilsvarende. Ikke kopier teksten til hendelser eller opprett et nytt tekstdokument for å flytte koblingen.
- Før ferdigmelding: kontroller at bare riktig dato og messe viser teksten, at nabosøndager og andre messespråk ikke arver den, og at samme dato/messe ikke har flere tekstdokumenter. Ved motstrid mellom forsidens søndag, eksplisitt dato og kalender: stopp og avklar, ikke flytt eller erstatt liturgiske tekster på gjetning.
...
- Messetekstformatering: bruk `{ "type": "refrain", "text": "Omkved uten responstegn" }` for responsoriesalmens omkved og `{ "type": "verse", "lines": ["Første verslinje", "Andre verslinje"] }` for hver strofe. Importeren setter inn ℟ foran omkvedet (fet kursiv), bevarer linjeskiftene og legger til en fet ℟ på egen linje etter hver strofe. Ikke skriv ℟ eller ℣ selv i disse teksttypene.
- Evangelievers med omkved–vers–omkved bruker `{ "type": "acclamation", "response": "Allelúia.", "lines": ["Herre, gjør våre hjerter åpne", "for din Sønns ord."] }`. Importeren legger til ℟/℣ og gjentar omkvedet; nettsiden viser hele blokken sentrert i kursiv, med bare ℟ og ℣ i fet skrift. Bruk kildens faktiske omkved, også utenfor alleluia-tiden. Utelat `response` når kilden ikke har omkved. Andre strukturer gjengis med vanlig Portable Text; ikke press dem inn i dette mønsteret.
- Bevar meningsbærende verslinjer og strofer fra visuell PDF-kontroll, men ikke kopier tilfeldige linjebrytinger i vanlig prosa. Ikke bruk Markdown-stjerner eller HTML i tekststrengene. Se komplett eksempel i `scripts/bulletin-manifest.example.json` (i Studio-mappen) og `docs/mass-text-formatting.md`. Eksemplets datoer, referanser og lesninger skal erstattes med den nye PDF-ens innhold. Tørrtesten avviser ugyldige teksttyper og varsler om sammenflatet liturgisk tekst. Kontroller omkved, strofer, fete responstegn og evangelievers visuelt på nettsiden, også på mobil, før ferdigmelding.
...
- Kunngjøringsrelevans: prøv alltid å sette en begrunnet `appliesUntil`. Eksplisitt periode i bladet vinner; ellers bruk frist, hendelsesdato eller tidspunktet en varslet endring er gjennomført. Skill mellom en varig ordning og hvor lenge kunngjøringen om endringen er aktuell. Ikke finn på en faktisk sluttdato for ordningen. Bare reelt åpne saker uten forsvarlig sluttdato kan utelate feltet; begrunn dette i importens ferdigmelding og vurder saken på nytt neste gang. Sett `status: resolved` når saken er uaktuell; ikke krev en oppdiktet historisk dato. Kunngjøringer har ingen generell aldersgrense. `urgent` bare ved kort, reell frist; `important` ved tydelig høy betydning; ellers `normal`.
