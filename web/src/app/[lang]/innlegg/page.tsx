...
export default async function InnleggPage({
  ...
  const category =
    query.category === "council" || query.category === "other"
      ? query.category
      : "all";
        ...
        {filters.map((filter) => (
          <Link
            ...
            href={
              filter.value === "all"
                ? "/innlegg"
                : `/innlegg?category=${filter.value}`
            }
            scroll={false}
            aria-current={category === filter.value ? "page" : undefined}
            className={buttonVariants({
              variant: category === filter.value ? "default" : "ghost",
              size: "sm",
            })}
          >
            {filter.label}
          ...
          >
            <div className="min-w-0 flex-1">
              ...
              <h2 className="mt-2 font-display text-xl font-semibold leading-snug text-foreground sm:text-2xl">
                ...
                >
                  <ContentLanguage
                    original={article.originalFields?.includes("title")}
                  >
